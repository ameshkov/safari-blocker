...
actor ToolbarData {
    ...
    func trackBlocked(on page: SFSafariPage, count: Int) async {
        ...
        let currentTabData = tabBlockedRequests[tabData.key]

        // If the URL is the same, we add the new count to the existing count.
        // Otherwise, we assume that the page has changed and the count should be set anew.
        if tabData.url == currentTabData?.url {
